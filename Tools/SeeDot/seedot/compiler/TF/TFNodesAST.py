...
class TFNodesAST:
	...
	def MatMul(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		if inp1Str == "Z":
			return (None, AST.Bop1(inp1AST, SeeDotParser.SPARSEMUL, inp2AST))	

		return (None, AST.Bop1(inp1AST, SeeDotParser.MUL, inp2AST))
	...
	def Placeholder(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		if(len(curNodeShapeLi)==0):
			return (None, AST.Float(extraNodeInfoDict[curNode.getName()][1]))
		return (None, AST.Decl(curNodeShapeLi, extraNodeInfoDict[curNode.getName()][1]))

	# TODO: Implement equality in SeeDot	
	...
	def Add(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		inputsRef = curNode.getInputsRef()
		assert(len(inputsRef) == 2)
		return (None, AST.Bop2(AST.ID(dictNodeNameToOutVarStr[inputsRef[0]]),
							SeeDotParser.ADD,
							AST.ID(dictNodeNameToOutVarStr[inputsRef[1]])
							))

	def isNumber(shape: list):
		for val in shape:
			if val>1:
				return False
		return True
	...
	def Mul(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		# basically if one of the input is a constant then make it MUL, otherwise make MULCIR
		if TFNodesAST.isNumber(extraNodeInfoDict[inputsRef[0]][0]) or TFNodesAST.isNumber(extraNodeInfoDict[inputsRef[1]][0]):
			op = SeeDotParser.MUL
		else:
			op = SeeDotParser.MULCIR	

		return (None, AST.Bop1(AST.ID(dictNodeNameToOutVarStr[inputsRef[0]]),
							SeeDotParser.MULCIR,
							AST.ID(dictNodeNameToOutVarStr[inputsRef[1]])
							))
	...
	def Neg(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		inputsRef = curNode.getInputsRef()
		assert(len(inputsRef) == 1)
		return (None, AST.Uop(SeeDotParser.SUB,
						   AST.ID(dictNodeNameToOutVarStr[inputsRef[0]])
						   ))
	...
	# TODO: support floor operation in SeeDot
	...
	# TODO: support division in SeeDot
	...
	#	inputsRef = curNode.getInputsRef()
	#	assert(len(inputsRef) == 2)
	#	realDivAST = AST.Bop1(AST.ID(dictNodeNameToOutVarStr[inputsRef[0]]),
	#						TFNodesAST.getOperatorsIdx('./'),
	#						AST.ID(dictNodeNameToOutVarStr[inputsRef[1]])
	#						)
	#	return (None, AST.Func(TFNodesAST.getOperatorsIdx('floor'), realDivAST))

	# Information of the range is extracted from the npy file
	...
	def Const(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		if tensorConstantVal is not None:
			...
			if curNodeDataType == Graph.DataTypeEnum.DT_INT32:
				dataPassed = AST.Int(tensorConstantVal)
			elif curNodeDataType == Graph.DataTypeEnum.DT_FLOAT:
				dataPassed = AST.Float(tensorConstantVal)
			else:
				assert False

			# Even including lenght 1 arrays as integers, since we do not support integer initialization yet
			# TODO: Add support for integer arrays		
			if (len(curNodeShape) == 0 or len(curNodeShape) == 1):
			...
			if curNodeDataType == Graph.DataTypeEnum.DT_INT32:
				#dataPassed = list(map(lambda x: AST.Int(x), tensor.getContentAsValArr()[:]))
				dataPassed = tensor.getContentAsValArr()[:]
			elif curNodeDataType == Graph.DataTypeEnum.DT_FLOAT:
				#dataPassed = list(map(lambda x: AST.Float(x), tensor.getContentAsValArr()[:]))
				# For now we are just supporting integer arrays? 
				dataPassed = tensor.getContentAsValArr()[:]
			else:
				assert False
			# TODO: This is wrong. We do not have array initialization in SeeDot
			retAST = AST.Array(curNodeShape, dataPassed)
		return (None, retAST)
	...
	def ApplyGradientDescent(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		return (inputTensor,  AST.Bop2(inputTensor, 
									SeeDotParser.ADD, 
									AST.Uop(SeeDotParser.SUB, 
										AST.Bop1(learningRate, 
											SeeDotParser.MULCIR, 
											deltaTensor))))
	...
	def Reshape(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		inputsRef = curNode.getInputsRef()
		assert(len(inputsRef) == 2)
		return (None, AST.Reshape(AST.ID(dictNodeNameToOutVarStr[inputsRef[0]]), extraNodeInfoDict[curNode.getName()][0]))
	...
	def MaxPool(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		return (None, AST.UninterpFuncCall(extraNodeInfoDict[curNode.getName()][0],
											...
											[AST.Int(kSizeH), AST.Int(kSizeW),
											 AST.Int(zPadH), AST.Int(zPadW), 
											 AST.Int(strideH), AST.Int(strideW),
											 AST.ID(dictNodeNameToOutVarStr[inputsRef[0]])]
											))
	...
	def ArgMax(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		inputsRef = curNode.getInputsRef()
		assert(len(inputsRef) == 2)
		# We ignore the second argument that denotes the axis
		# Currently only support for 1D tensor
		return (None, AST.Func(SeeDotParser.ARGMAX, 
								 AST.ID(dictNodeNameToOutVarStr[inputsRef[0]])))

	def Tanh(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		inputsRef = curNode.getInputsRef()
		assert(len(inputsRef) == 1)
		return (None, AST.Func(SeeDotParser.TANH, AST.ID(dictNodeNameToOutVarStr[inputsRef[0]])))
	...
	def LogSoftmax(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		reduceAST = AST.Reduce(expAST, AST.Int(-1), SeeDotParser.ADD)
		return (None, AST.Bop2(AST.ID(dictNodeNameToOutVarStr[inputsRef[0]]), 
							SeeDotParser.ADD,
							AST.Uop(SeeDotParser.SUB, AST.Func(TFNodesAST.getOperatorsIdx('log'), reduceAST))))
	...
	def SoftmaxCrossEntropyWithLogits(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		retAST.expr = AST.Let(AST.ID('temp_1'), 
							AST.Uop(SeeDotParser.SUB,  
									AST.Reduce(AST.Bop1(labelsInpt, 
														SeeDotParser.MULCIR, 
	...
	def Squeeze(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		return (None, AST.UninterpFuncCall(extraNodeInfoDict[curNode.getName()][0],
											...
											list(map(lambda x : AST.Int(x), squeezeDims)) + 
											[
											AST.ID(dictNodeNameToOutVarStr[inputsRef[0]])
											]
											))
	...
	def StridedSlice(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		return (None, AST.UninterpFuncCall(extraNodeInfoDict[curNode.getName()][0],
								 ...
								 AST.ID(dictNodeNameToOutVarStr[inputsRef[1]]),
								 AST.ID(dictNodeNameToOutVarStr[inputsRef[2]]),
								 AST.ID(dictNodeNameToOutVarStr[inputsRef[3]])]))	

	def Rank(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		inputsRef = curNode.getInputsRef()
		print(inputsRef)
		assert(len(inputsRef) == 1)
		return (None, AST.UninterpFuncCall(extraNodeInfoDict[curNode.getName()][0],
	...
	def Range(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		inputsRef = curNode.getInputsRef()
		print(inputsRef)
		assert(len(inputsRef) == 3)
		return (None, AST.Int(-124))
		#return (None, AST.UninterpFuncCall(extraNodeInfoDict[curNode.getName()][0],
		#							 "Range", 
		#						  [AST.ID(dictNodeNameToOutVarStr[inputsRef[0]]), 
		#						 AST.ID(dictNodeNameToOutVarStr[inputsRef[1]]),
	...
	def Transpose(graph : Graph.Graph, curNode : Graph.Node, dictNodeNameToOutVarStr : dict, extraNodeInfoDict : dict):
		...
		print(inputsRef)
		assert(len(inputsRef) == 2)
		# we ignore the second input for now, and provide the basic transpose feature
		return (None, AST.Transp(AST.ID(dictNodeNameToOutVarStr[inputsRef[0]])))	
